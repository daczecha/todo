...
export function sideBar()
{
    ...
    $completedTasks.id = 'completed_tasks';
    $completedTasks.innerText = `Completed Tasks : ${completedTasks.length}`
    ...
    for(let i = 1; i < projectArray.length; i++){
        ...
        $removeProjectBtutton.innerText = 'Remove';
    ...
    for (const btn of $removeProjectButtonArray) {
        btn.addEventListener('click', function() {
            ...
            if(index > -1){
                ...
                document.getElementById('side_bar').removeChild(document.getElementById(`project_${index}`));
                if(projectArray.length === 1){
                    main(projectArray[0]);
                }else{
                    $app.removeChild(document.getElementById('main'));
                    main(projectArray[projectArray.length-1]);
                    resetProjects();
                }
            }
        });
...
function resetProjects(){
    const $removeProjectButtonArray = document.querySelectorAll('.remove-project-button');
    for (let i = 1; i < $removeProjectButtonArray.length; i++) {
        $removeProjectButtonArray[i].id = `remove_button_${i}`;
        $removeProjectButtonArray[i].parentNode.id = `project_${i}`;
    }
}   